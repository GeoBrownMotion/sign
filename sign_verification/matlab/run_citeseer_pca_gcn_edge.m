function run_citeseer_pca_gcn_edge()
%RUN_CITESEER_PCA_GCN_EDGE  Edge perturbation verification for CiteSeer PCA-32 GCN.
% delta=0 (no feature perturbation); 1/3/5 random edges removed per node.

    here = fileparts(mfilename('fullpath'));
    setup_cora(fullfile(here, '..', '..', 'cora'));

    model_json = fullfile(here, '..', '..', 'results', 'citeseer_pca', 'gcn', 'model_export.json');
    data_json  = fullfile(here, '..', '..', 'results', 'citeseer_pca', 'gcn', 'data_export.json');
    ...
    raw = data{1, 'target_label'}; if iscell(raw); raw = raw{1}; end; targets = double(raw);
    raw = data{1, 'output'}; if iscell(raw); raw = raw{1}; end; Y_ref = double(raw);

    num_nodes = size(X, 1);
    feat_dim = size(X, 2);
    fprintf('CiteSeer PCA-32 GCN edge pert: %d nodes, %d features\n', num_nodes, feat_dim);
    ...
    num_test = min(100, numel(correct_nodes));
    test_nodes = correct_nodes(1:num_test);
    fprintf('Verifying %d correctly-classified nodes\n', num_test);

    num_pert_list = [1, 3, 5];
    results = struct();
    for ei = 1:numel(num_pert_list)
        results(ei).pert = num_pert_list(ei);
        results(ei).V = 0; results(ei).U = 0; results(ei).E = 0; results(ei).S = 0;
    ...
    for ni = 1:numel(test_nodes)
        ...
        for ei = 1:numel(num_pert_list)
            np = num_pert_list(ei);
            try
                [G_pert, idxPert] = aux_perturb_edges(G_sub, np);
                if numel(idxPert) < np
    ...
    fprintf('\n=== CiteSeer PCA-32 GCN Edge Perturbation ===\n');
